...
local ip = "127.0.0.1" -- make sure Domoticz allows password free call on that IP and does not block it
local port = "8080" -- the local port on which to access Domoticz

-- Domoticz devices used by script.
-- virtual devices MUST be manually created before running script)
-- single devices
  local thermostat = "Thermostat" -- virtual, type switch
  local forcedheating = "Forced_Heating" -- virtual, type switch
  local tempthermostat = "Temp_Thermostat" -- virtual, type temperature
-- tables of device(s) allowing multiple sensors and heaters device(s)
...
local function CreateUserVariable(varname, vartype, value)
  ...
  local url = "http://%s:%s/json.htm?type=command&param=saveuservariable&vname=%s&vtype=%d&vvalue=%s"
  table.insert(commandArray, { ["OpenURL"] = string.format(url, ip, port, varname, vartype, value)})
end
...
local function variable_get(variable)
  ...
  if value then
    ...
    print("SVT: creating user variable " .. variable.name) 
...
local function GetTemp(sensors)
  ...
  for _, device in pairs(sensors) do
    if datetoepoch(otherdevices_lastupdate[device]) - os.time() <= dirtydata then
...
local function GetPause()
  ...
  local delai = (os.time() - lastupdate) >= variable_get(uv.PauseDelay)
...
local function Heat(action)
  ...
  if action == "On" then
    ...
    data.ForcedHeating = 0 --variable_set(uv.ForcedHeating, 0)
  end
  SaveData()
end
  ...
  if intemp then
    ...
    if otherdevices[thermostat] == "On" then
      ...
      if otherdevices[forcedheating] == "On" then -- we deal with forced heating situation
        if otherdevices_lastupdate[forcedheating] > data.ForcedHeatingTime then -- newly set
      ...
      elseif not(pause) and pausechanged then -- the pause just stopped... reset normal situation but no learning
        ...
        elseif intemp >= setpoint + deltamax then
          ...
          data.Heating = 0 -- AutoChangeOver(true)
--          data.LastCalc = now
        end
      end
